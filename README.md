...
- [환경 변수](#환경-변수)
- [API](#api)
- [대시보드 지표와 선정 근거](#대시보드-지표와-선정-근거) (상세: [METRICS.md](METRICS.md))
- [안정성 설계](#안정성-설계)
- [테스트](#테스트)
- [프로젝트 구조](#프로젝트-구조)
...
- **투자 판단용**: 실시간가 + 1분봉 캔들 차트, 24h 변동률/고가/저가, 24h 거래량(quote volume),
  매수/매도 체결 비율 (`is_buyer_maker` 기반)
- **운영 모니터링용**: WS 연결 상태 + 마지막 수신 후 경과 시간, 재연결 횟수/에러 카운트,
  마지막 백필 시각/커버 범위

투자 판단용 지표는 "지금 이 종목이 사고팔 만한 상태인가"를, 운영 모니터링용 지표는
"지금 이 수치를 신뢰할 수 있는가"를 답하도록 구성했습니다. 데이터가 멈춘 상태에서 가격만
보여주면 오판을 유발할 수 있어, 두 축을 하나의 화면에 같이 두었습니다. 각 지표를 선택한
상세 근거는 [`METRICS.md`](METRICS.md)에 정리했습니다.
